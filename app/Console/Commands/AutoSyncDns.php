        ...
        foreach ($domains as $domain) {
            ...
            if ($record->type !== 'A') {
                ...
                if (!isset($delResult['success']) || !$delResult['success']) {
                    $this->error("  [FAIL] {$domain->domain_name}: Gagal hapus {$oldType} — " . ($delResult['errors'][0]['message'] ?? 'Unknown'));
                    DnsUpdateLog::create([
                        'tracked_domain_id' => $domain->id,
                        'zone_name' => $zone->name,
                        'record_name' => $record->name,
                ...
                if (!isset($createResult['success']) || !$createResult['success']) {
                    $this->error("  [FAIL] {$domain->domain_name}: Gagal buat A record — " . ($createResult['errors'][0]['message'] ?? 'Unknown'));
                    DnsUpdateLog::create([
                        'tracked_domain_id' => $domain->id,
                        'zone_name' => $zone->name,
                        'record_name' => $record->name,
                ...
                $this->line("  [CONV] {$domain->domain_name}: {$oldType} → A ({$ip})");
                $successCount++;
                continue;
            }
